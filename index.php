<?php
// بخش‌های موجود index.php (به طور خلاصه)
...
// کدهای موجود برای مدیریت بات...

// افزودن قابلیت تغییر لوکیشن
if (isset($data['data']) && strpos($data['data'], 'changelocation_') === 0) {
    $username = str_replace('changelocation_', '', $data['data']);
    $user_id = $data['from']['id'];

    // بررسی فاکتور کاربر
    $stmt = $pdo->prepare("SELECT * FROM invoice WHERE username = ? AND id_user = ? AND status IN ('active', 'end_of_time', 'end_of_volume')");
    $stmt->execute([$username, $user_id]);
    $invoice = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$invoice) {
        sendmessage($user_id, $textbotlang['users']['error']['invalid_service'], null, 'HTML');
        exit;
    }

    // دریافت لیست پنل‌های موجود با ظرفیت کمتر از 100 کلاینت
    $stmt = $pdo->prepare("SELECT name_panel FROM marzban_panel WHERE status = 'active' AND client_count < 100");
    $stmt->execute();
    $panels = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if (empty($panels)) {
        sendmessage($user_id, $textbotlang['users']['error']['no_panels_available'], null, 'HTML');
        exit;
    }

    // ایجاد دکمه‌های انتخاب پنل
    $keyboard = [];
    foreach ($panels as $panel) {
        $keyboard[] = [['text' => $panel['name_panel'], 'callback_data' => 'selectlocation_' . $username . '_' . $panel['name_panel']]];
    }
    $Response = json_encode(['inline_keyboard' => $keyboard]);

    sendmessage($user_id, $textbotlang['users']['changelocation']['select_panel'], $Response, 'HTML');
    exit;
}

// انتخاب پنل جدید
if (isset($data['data']) && strpos($data['data'], 'selectlocation_') === 0) {
    $parts = explode('_', str_replace('selectlocation_', '', $data['data']));
    $username = $parts[0];
    ...
    // بررسی پنل جدید و ظرفیت آن
    $stmt = $pdo->prepare("SELECT * FROM marzban_panel WHERE name_panel = ? AND status = 'active' AND client_count < 100");
    $stmt->execute([$new_location]);
    $new_panel = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$new_panel) {
        sendmessage($user_id, $textbotlang['users']['error']['panel_full_or_invalid'], null, 'HTML');
        exit;
    }

    // دریافت اطلاعات کاربر از پنل فعلی
    $current_location = $invoice['Service_location'];
    $user_data = $ManagePanel->DataUser($current_location, $username);

    if ($user_data['status'] == 'Unsuccessful') {
        sendmessage($user_id, $textbotlang['users']['error']['user_not_found'], null, 'HTML');
        exit;
    }
    ...
    try {
        ...
        // حذف کاربر از پنل فعلی
        $ManagePanel->RemoveUser($current_location, $username);

        // افزودن کاربر به پنل جدید
        $add_result = $ManagePanel->AddUser($new_location, $username, $user_data['data_limit'], $user_data['expire']);

        if ($add_result['status'] == 'success') {
            // به‌روزرسانی لوکیشن در دیتابیس
            $stmt = $pdo->prepare("UPDATE invoice SET Service_location = ? WHERE username = ?");
            $stmt->execute([$new_location, $username]);

            // به‌روزرسانی تعداد کلاینت‌ها
            $stmt = $pdo->prepare("UPDATE marzban_panel SET client_count = client_count - 1 WHERE name_panel = ?");
            ...
            sendmessage($user_id, sprintf($textbotlang['users']['changelocation']['success'], $username, $new_location), null, 'HTML');

            // گزارش به کانال (در صورت تنظیم)
            $setting = select("setting", "*");
            if (strlen($setting['Channel_Report']) > 0) {
                sendmessage($setting['Channel_Report'], sprintf($textbotlang['Admin']['Report']['location_changed'], $username, $current_location, $new_location), null, 'HTML');
            }
        } else {
            $pdo->rollBack();
            sendmessage($user_id, $textbotlang['users']['error']['change_failed'], null, 'HTML');
        }
    } catch (Exception $e) {
        // لغو تراکنش در صورت خطا
        $pdo->rollBack();
        error_log("Error changing location for user $username: " . $e->getMessage());
        sendmessage($user_id, $textbotlang['users']['error']['change_failed'], null, 'HTML');
    }
    exit;
}

// افزودن دکمه تغییر لوکیشن به منوی سرویس‌ها
function addChangeLocationButton($username) {
    global $textbotlang;
    return [
        'inline_keyboard' => [
            [
                ['text' => $textbotlang['users']['extend']['title'], 'callback_data' => 'extend_' . $username],
                ['text' => $textbotlang['users']['changelocation']['title'], 'callback_data' => 'changelocation_' . $username]
            ]
        ]
    ];
}
